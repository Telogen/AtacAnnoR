...
Meta-program matrix can be calculated using `get_nmf_embedding()` function in AtacAnnoR, the default factor number is set to 50.
...
### Step1: Identification of reference cell type-specific marker genes
...
### Step2: Determination of candidate cell type labels for each query cell 
...
### Step3: Validation of candidate cell type labels  
...
`plot_cell_distribution()` can be used to plot the distribution of GMSS and NMSS for one predicted cell type.
...
get_benchmark(seed_meta$true,seed_meta$kendall_pred)    # seed cell candidates accuracy
```

### Step2: Final prediction
